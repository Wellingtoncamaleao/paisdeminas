...
function atualizarOutrosJogadores(delta) {
  ...
  // Smooth interpolation dos avatares (independente de poll)
  var k = Math.min(delta * 6, 1);
  for (var id in avataresOutros) {
    var av = avataresOutros[id];
    av.mesh.position.lerp(av.alvoPos, k);
...
// === Avatar simplificado pra outros jogadores ===
// Mesmo formato basico do humanoide do jogador, mas com cor de camisa do hash
function criarAvatarOutroJogador(corCamisa, nome) {
  ...
  // Calca (cilindro grosso)
  var pernas = new THREE.Mesh(
    new THREE.CylinderGeometry(0.18, 0.16, 0.7, 6),
    new THREE.MeshLambertMaterial({ color: 0x3d2812 })
  );
